...
class Solution:
    ...
    def rightSideView(self, root: Optional[TreeNode]) -> List[int]:
        ...
        if root is None:
            return []
        partial_correct_view = [root.val] + self.special_traverse(root.right) 
        return partial_correct_view + self.special_traverse(root.left)[len(self.special_traverse(root.right)):]
